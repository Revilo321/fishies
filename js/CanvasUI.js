  ...
  onSelect(index) {
    ...
    switch (index) {
      ...
      case 30: //switch keyboard
        ...
        break
      case 28: //backspace
        this.linkedText = this.linkedText.substring(
          0,
          this.linkedText.length - 1
        )
        change = true
        break
      case 20: //shift
...
class CanvasUI {
  constructor(content, config, object, engine) {
    ...
    Object.entries(this.config).forEach(([name, value]) => {
      if (typeof value === 'object' && name !== 'panelSize') {
        //} && !(value instanceof WebGLRenderer) && !(value instanceof Scene) ){
        const pos =
    ...
    if (inputs.length > 0) {
      ...
      console.log(
        `CanvasUI create keyboard 1 theta=${theta.toFixed(
          2
        )} offset=${this.vecToStr(this.tmpVec)}`
      )
      ...
      console.log(
        `CanvasUI create keyboard 2 theta=${theta.toFixed(
          2
        )} offset=${this.vecToStr(this.tmpVec1)}`
      )
      vec3.add(this.tmpVec, this.tmpVec, this.tmpVec1)
      console.log(
        `CanvasUI create keyboard 3 theta=${theta.toFixed(
          2
        )} offset=${this.vecToStr(this.tmpVec)}`
      )
    ...
    if (content === undefined) {
      ...
      if (btns.length > 0) {
        ...
        const collision = this.object.addComponent('collision', {
          collider: 2,
          extents,
  ...
  initControllers(s) {
        ...
        if (elm.type == 'button') {
          ...
          if (this.keyboard) {
            if (this.keyboard.visible) {
              ...
              const y =
                (0.5 -
                  (elm.position.y + elm.height + this.config.body.padding) /
                    this.config.height) *
                this.panelSize.height
              const h =
                Math.max(this.panelSize.width, this.panelSize.height) / 2
    ...
    function onSelectStart(event) {
      ...
      if (
        ...
        this.selectedElements[index].overflow == 'scroll'
      ) {
        const elm = this.selectedElements[index]
  ...
  getElementAtLocation(x, y) {
    const elms = Object.entries(this.config).filter(([name, elm]) => {
      if (typeof elm === 'object' && name !== 'panelSize' && name !== 'body') {
        // && !(elm instanceof WebGLRenderer) && !(elm instanceof Scene)){
        const pos = elm.position
        const width = elm.width !== undefined ? elm.width : this.config.width
        const height =
          elm.height !== undefined ? elm.height : this.config.height
        return (
  ...
  handleController(controller, index) {
    ...
    const intersects = WL.scene.rayCast(
      this.tmpVec,
      this.tmpVec1,
      this.collisionGroup,
      100
  ...
  update() {
    ...
    Object.entries(this.content).forEach(([name, content]) => {
      ...
      if (display !== 'none') {
        ...
        const hover =
          (this.selectedElements[0] !== undefined &&
            this.selectedElements[0] === config) ||
          (this.selectedElements[1] !== undefined &&
  ...
  set position(value) {
    ...
    if (!(value instanceof Float32Array)) {
      console.error(
        'CanvasUI trying to set the object position using a parameter that is not a Float32Array'
      )
      return
    }
  ...
  set quaternion(value) {
    ...
    if (!(value instanceof QUaternion)) {
      console.error(
        'CanvasUI trying to set the object quaternion using a parameter that is not a THREE.Quaternion'
      )
      return
    }
